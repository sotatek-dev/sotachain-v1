...
use pallet_evm::AddressMapping as PAddressMapping;
// use orml_traits::currency::TransferAll;
use node_primitives::{evm::EvmAddress, to_bytes, AccountIndex};
...
impl<T: Config> Pallet<T> {
	...
	fn evm_account_domain_separator() -> [u8; 32] {
		...
		domain_seperator_msg.extend_from_slice(keccak256!("Sota EVM claim")); // name
...
/// use native pallet-evm, so we need this trait
impl<T: Config> PAddressMapping<T::AccountId> for EvmAddressMapping<T>
where
	T::AccountId: IsType<AccountId32>,
{
	// Returns the AccountId used to generate the given EvmAddress.
